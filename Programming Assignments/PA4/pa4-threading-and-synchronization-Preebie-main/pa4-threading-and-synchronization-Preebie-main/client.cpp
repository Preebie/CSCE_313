...
void file_function(BoundedBuffer *requestbuffer, FIFORequestChannel *chan, string file, int bytes, string filestring)
{
...
void worker_thread_function(FIFORequestChannel *chan, BoundedBuffer *request_buf, BoundedBuffer *response_buf, mutex *muter, FILE *fp, string filename)
{
    while (true)
    {
        char arr[1024];
...
int main(int argc, char *argv[])
{
    ...
    if (file_transfer)
    {
        ...
        FIFORequestChannel *chanw[w];
        struct timeval start, end;
        gettimeofday(&start, 0);
        for (int i = 0; i < w; i++)
        {
            MESSAGE_TYPE chan_name = NEWCHANNEL_MSG;
            chan->cwrite(&chan_name, sizeof(MESSAGE_TYPE));
            char buffer[30];
            chan->cread(&buffer, sizeof(buffer));
            FIFORequestChannel *channel = new FIFORequestChannel(buffer, FIFORequestChannel::CLIENT_SIDE);
            chanw[i] = channel;
        }
